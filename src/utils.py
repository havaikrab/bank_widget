...
def get_amount_rubles(transaction: dict) -> float:
    ...
    if (
            ...
            for i in range(len(transaction["date"])):
                if not transaction["date"][i].isdigit():
                    if transaction["date"][i] != symbol_index_dict[i]:
                        utils_logger.error(f'Дата транзакции имеет некорректный формат, работа завршена с ошибкой')
        ...
        utils_logger.error(f'Некорректный формат данных, работа завершена с ошибкой')
